...
internal class SelectView(reactContext: ThemedReactContext) :
  InlineComposeView(reactContext, TAG) {

  companion object {
    private const val TAG = "SelectView"
  }
  ...
  private fun parseItems(json: String): List<SelectItem> {
    return try {
          ...
          DropdownMenuItem(
            ...
            onClick = {
              ...
              if (item.value != selectedValue) {
                dispatchEvent("topValueChange", mapOf("value" to item.value))
              }
            },
  ...
  data class SelectItem(val value: String, val label: String)
}
